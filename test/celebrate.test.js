...
describe('celebrate()', () => {
  ...
  it(`validates ${segment}`, () => {
    ...
    middleware(req, null, (err) => {
      ...
      expect(err.error.details[0].message).toBe(message);
    });
  });
});
  ...
  it('errors on the first validation problem (params, query, body)', () => {
    ...
    middleware({
      ...
      expect(isCelebrate(err)).toBe(true);
      expect(err.error.details[0].message).toBe('"end" is not allowed');
    });
  });
  ...
  it('works with Joi.extend()', () => {
    ...
    middleware({
      ...
      expect(isCelebrate(err)).toBe(true);
      expect(err.error.details[0].message).toBe('"first" must equal "john"');
    });
  });
  ...
  it('honors the escapeHtml Joi option', () => {
    ...
    middleware({
      ...
      expect(isCelebrate(err)).toBe(true);
      expect(err.error.details[0].message).toBe('"accept" with value "application/json" fails to match the required pattern: /xml/');
    });
  });
  ...
  it('supports static reference', () => {
    ...
    middleware({
      ...
      expect(isCelebrate(err)).toBe(true);
      expect(err.error.details[0].message).toBe('"id" must be one of [context:id]');
    });
  });

  it('validates with the request as context', () => {
    expect.assertions(2);
    const middleware = celebrate({
      params: {
    ...
    middleware({
      ...
      expect(isCelebrate(err)).toBe(true);
      expect(err.error.details[0].message).toBe('"id" must be one of [context:params.userId]');
    });
  });
});
...
describe('errors()', () => {
  ...
  it('only includes key values if joi returns details', () => {
    ...
    middleware({
      ...
      err.error.details = null; // eslint-disable-line no-param-reassign
      handler(err, undefined, res, next);
    });
  });
});
