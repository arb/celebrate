...
internals.partialValidate = (steps, req) => steps.reduce((chain, validate) => chain.then(() => validate(req)
  ...
  .catch((e) => {
    ...
    error.include(validate.segment, e);
    throw error;
...
// TODO: extend Map and validate set()
...
exports.CelebrateError = class extends Error {
  ...
  include(segment, joiError) {
    // Assert.ok(joiError.isJoi, 'joiError must be a joi ValidationError');
...
exports.celebrate = (_requestRules, joiOpts = {}, opts = {}) => {
  ...
  const middleware = (req, res, next) => {
    ...
    const v = internals.validateFns[finalOpts.mode];
